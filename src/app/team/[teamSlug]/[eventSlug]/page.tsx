...
function Avatar({
  ...
  if (image) {
    if (image.startsWith("data:")) {
      ...
      return <img src={image} alt={name ?? ""} style={{ width: size, height: size }} className="rounded-full object-cover ring-1 ring-[#2e2e2e]" />;
    }
    return (
      <Image src={image} alt={name ?? ""} width={size} height={size}
        className="rounded-full object-cover ring-1 ring-[#2e2e2e]" style={{ width: size, height: size }} />
    );
  }
  return (
    <div className="flex items-center justify-center rounded-full bg-[#c4956a] font-semibold text-white ring-1 ring-[#2e2e2e]"
    ...
    <div style={{ minHeight: "100vh", background: "#0c0c0c", color: "#fff" }}>
      <div className="mx-auto max-w-2xl px-4 py-12 md:py-16">
        ...
        <div className="mb-8 text-center">
          <div className="mb-4 flex justify-center">
            <div className="flex h-14 w-14 items-center justify-center rounded-2xl"
              ...
              <Users className="h-7 w-7" style={{ color: "#c4956a" }} />
            </div>
          </div>
          <h1 className="text-xl font-bold text-white">{team.name}</h1>
          {team.description && (
            <p className="mt-1 text-sm" style={{ color: "#9a9a9a" }}>{team.description}</p>
          )}
          {/* Team member avatars */}
          <div className="mt-3 flex items-center justify-center gap-1">
            ...
            {team.members.length > 6 && (
              <span className="ml-1 text-xs" style={{ color: "#555" }}>+{team.members.length - 6}</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid #1e1e1e" }}>
          {/* Event info sidebar */}
          <div style={{ background: "#111", borderBottom: "1px solid #1e1e1e", padding: "24px 28px" }}>
            <div className="flex items-start gap-3">
              <div className="h-3 w-1 rounded-full mt-1.5 shrink-0" style={{ background: eventType.color ?? "#c4956a" }} />
              <div>
                <h2 className="text-lg font-bold text-white">{eventType.title}</h2>
                <div className="mt-2 flex flex-wrap items-center gap-3 text-xs" style={{ color: "#9a9a9a" }}>
                  ...
                  <span className="flex items-center gap-1.5">
                    {loc.icon}
                    {loc.label}
                    {loc.detail && <span style={{ color: "#555" }}>· {loc.detail}</span>}
                  </span>
                  <span className="flex items-center gap-1.5">
                ...
                {eventType.description && (
                  <p className="mt-3 text-sm leading-relaxed" style={{ color: "#9a9a9a" }}>
                    {eventType.description}
                  </p>
                )}
              </div>
