 ...
 * Exchanges the MS auth code for tokens, stores encrypted in CalendarIntegration.
 */
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL("/dashboard/integrations?connected=teams", APP_URL),
    );
  } catch (err) {
    console.error("[GET /api/integrations/teams/callback]", err);
    return NextResponse.redirect(
      new URL("/dashboard/integrations?error=teams_server_error", APP_URL),
    );
  }
}
